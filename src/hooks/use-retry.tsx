...
const useRetry = ({
  ...
  const [cookies, setCookie, removeCookie] = useCookies([`blockTimeRemaining-${userKey}`, `isBlocked-${userKey}`]);
  ...
  const [blockTimeRemaining, setBlockTimeRemaining] = useState(blockTimeRemainingFromCookie);
  ...
  const startBlock = useCallback(() => {
    ...
    setCookie(`blockTimeRemaining-${userKey}`, releaseTime.toString(), { path: '/' });
  }, [blockTime, setCookie, userKey]);
  ...
  useEffect(() => {
    ...
    if (isBlocked) {
      ...
      const releaseTime = parseInt(cookies[`blockTimeRemaining-${userKey}`] || '0');
  ...
  }, [isBlocked, cookies, resetBlock, userKey]);

  useEffect(() => {
    // If the page is refreshed or revisited, check if the user is still blocked
    const currentTime = getCurrentTime();
    if (cookies[`isBlocked-${userKey}`] === 'true' && cookies[`blockTimeRemaining-${userKey}`]) {
